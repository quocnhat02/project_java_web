...
const ChangeQuantityOfBook: React.FC<{ book: BookModel; deleteBook: any }> = (
  ...
  async function increaseAvailable() {
    const url = `http://localhost:8080/api/admin/secure/increase/book/available/?bookId=${props.book?.id}`;
    const requestOptions = {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
    ...
    const availableUpdateResponse = await fetch(url, requestOptions);
    if (!availableUpdateResponse.ok) {
      throw new Error('Something went wrong!');
    }
    setRemaining(remaining + 1);
  }
  ...
  return (
    <div className='card mt-3 shadow p-3 mb-3 bg-body rounded'>
      <div className='row g-0'>
        ...
        <button
          className='m1 btn btn-md btn-success'
          onClick={increaseAvailable}
        >
          Increase Quantity
        </button>
      </div>
